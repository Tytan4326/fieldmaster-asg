...
const GAME_MODES = {
  ...
  CAPTURE_FLAG:{name:'Capture the Flag',description:'Flagi, bazy i bezpieczny powrót.',icon:'⚑',defaults:{hitsToRespawn:1,respawnSeconds:40,respawnZoneRequired:true,lives:0,scoreLimit:5,roundMinutes:90,fovRange:150,fovAngle:65,modeRules:{flagReturnSeconds:120,capturesToWin:3,carrierVisible:true}},fields:[['flagReturnSeconds','Automatyczny powrót flagi (s)',10,1800],['capturesToWin','Flagi do zwycięstwa',1,50],['carrierVisible','Widoczny niosący flagę','boolean']]},
  ...
  CONVOY_AMBUSH:{name:'Konwój / Zasadzka',description:'Eskorta prowadzi konwój przez checkpointy.',icon:'▰',defaults:{hitsToRespawn:1,respawnSeconds:60,respawnZoneRequired:true,lives:3,scoreLimit:5,roundMinutes:150,fovRange:150,fovAngle:65,modeRules:{convoyLives:3,checkpointCount:5,ambushDelayMinutes:10}},fields:[['convoyLives','Wytrzymałość konwoju',1,20],['checkpointCount','Liczba checkpointów',1,30],['ambushDelayMinutes','Czas przygotowania zasadzki (min)',0,120]]},
...
let ui = { view: pathView || params.get('view') || 'join', adminTab:'dashboard', settingsTab:'session', staffTab:'overview', playerTab:'status', joinStep:1, joining:false, team:null, callsign:'', sessionCode:params.get('code')||state.game.code, adminGames:[], staffAccounts:[], staffAccountsLoaded:false, consent:false, rules:false, locationConsent:false, installPrompt:null, installed:window.matchMedia?.('(display-mode: standalone)').matches||window.navigator.standalone===true, gpsStatus:'NOT_TESTED', gpsAccuracy:null, gpsMessage:'', testLocation:null, boundaryDraft:null, zoneDrafts:null, zoneDraftGameId:null, selectedZoneId:null, selectedMode:state.game.mode, eventFilter:'ALL', eventSearch:'', fieldMode:localStorage.getItem('fm-field-mode')==='1' };
...
function connectRealtime(token,gameId){
  ...
  socket.on('message:new',message=>{state.messages=state.messages.filter(item=>item.id!==message.id);state.messages.unshift(message);save(false);toast('Nowy komunikat',message.body);beep(720,.12);scheduleRender();});
}
...
function featuresSettingsHtml(){const groups=[...new Set(FEATURE_DEFS.map(item=>item[4]))];return `<form class="panel" id="feature-settings-form"><div class="panel-head"><div class="panel-title">Moduły aplikacji</div><div class="panel-sub">${FEATURE_DEFS.length} funkcji w rozwijanych kategoriach</div></div><div class="accordion-stack feature-accordions">${groups.map((group,index)=>`<details class="settings-accordion" ${index<2?'open':''}><summary>${group}<span>${FEATURE_DEFS.filter(item=>item[4]===group&&enabled(item[0])).length}/${FEATURE_DEFS.filter(item=>item[4]===group).length} aktywnych</span></summary><div class="feature-grid">${FEATURE_DEFS.filter(item=>item[4]===group).map(([key,label,description,major])=>`<label class="feature-toggle"><input type="checkbox" data-feature="${key}" ${enabled(key)?'checked':''}><span class="feature-switch"></span><span><b>${esc(label)} ${major?'<em>KLUCZOWA</em>':''}</b><small>${esc(description)}</small></span></label>`).join('')}</div></details>`).join('')}</div><div class="settings-savebar"><span>Zmiany dotyczą tylko sesji ${esc(state.game.code)}</span><button class="btn btn-primary">Zapisz funkcje</button></div></form>`;}
function accountsSettingsHtml(){const accounts=ui.staffAccountsLoaded?ui.staffAccounts:(state.staff||[]),defaultPermissions=['VIEW_ALL_PLAYERS','VIEW_FOV','VIEW_SOS','SEND_ALL_MESSAGES','SEND_TEAM_MESSAGES','SEND_DIRECT_MESSAGES','RECEIVE_PLAYER_MESSAGES','ACK_SOS'];return `<div class="accounts-layout"><details class="panel account-create" open><summary>+ Utwórz konto personelu</summary><form id="staff-create-form"><div class="settings-body"><div class="settings-row"><div class="field"><label class="label">LOGIN</label><input class="input" id="staff-username" minlength="3" maxlength="32" required></div><div class="field"><label class="label">KRYPTONIM</label><input class="input" id="staff-callsign" maxlength="32" required></div><div class="field"><label class="label">FUNKCJA</label><input class="input" id="staff-title" value="Dowódca"></div><div class="field"><label class="label">HASŁO TYMCZASOWE</label><input class="input" id="staff-password" type="password" minlength="8" required></div><div class="field"><label class="label">PRZYPISANIE</label><select class="select" id="staff-team"><option value="ALL">Wszystkie strony</option><option value="SERE">SERE</option><option value="OPFOR">OPFOR</option></select></div></div><div class="permission-grid">${STAFF_PERMISSIONS.map(([key,label,group])=>`<label><input type="checkbox" data-new-permission="${key}" ${defaultPermissions.includes(key)?'checked':''}><span><b>${label}</b><small>${group}</small></span></label>`).join('')}</div><button class="btn btn-primary">Utwórz konto</button></div></form></details><section class="panel"><div class="panel-head"><div><div class="panel-title">Konta w sesji (${accounts.length})</div><div class="panel-sub">login personelu: kod ${esc(state.game.code)} + nazwa użytkownika + hasło</div></div><button class="btn btn-sm" data-action="staff-refresh">↻ Odśwież</button></div>${!ui.staffAccountsLoaded?'<div class="empty">Pobieranie kont…</div>':accounts.length?accounts.map(staffAccountCard).join(''):'<div class="empty">Nie utworzono jeszcze kont personelu.</div>'}</section></div>`;}
function staffAccountCard(account){return `<details class="staff-account ${account.active?'':'disabled'}"><summary><div class="avatar">${esc(account.callsign.slice(0,2).toUpperCase())}</div><span><b>${esc(account.callsign)}</b><small>${esc(account.title)} · login: ${esc(account.username)} · ${account.team}</small></span><em>${account.active?'AKTYWNE':'WYŁĄCZONE'}</em></summary><div class="staff-account-body"><div class="settings-row"><div class="field"><label class="label">KRYPTONIM</label><input class="input" id="staff-callsign-${account.id}" value="${esc(account.callsign)}"></div><div class="field"><label class="label">FUNKCJA</label><input class="input" id="staff-title-${account.id}" value="${esc(account.title)}"></div><div class="field"><label class="label">DRUŻYNA</label><select class="select" id="staff-team-${account.id}"><option value="ALL" ${account.team==='ALL'?'selected':''}>Wszystkie</option><option value="SERE" ${account.team==='SERE'?'selected':''}>SERE</option><option value="OPFOR" ${account.team==='OPFOR'?'selected':''}>OPFOR</option></select></div><label class="account-active"><input type="checkbox" data-staff-active="${account.id}" ${account.active?'checked':''}> konto aktywne</label></div><div class="permission-grid compact">${STAFF_PERMISSIONS.map(([key,label])=>`<label><input type="checkbox" data-staff-permission="${account.id}:${key}" ${(account.permissions||[]).includes(key)?'checked':''}><span>${label}</span></label>`).join('')}</div><div class="staff-actions"><input class="input" id="staff-password-${account.id}" type="password" minlength="8" placeholder="Nowe hasło (opcjonalnie)"><button class="btn btn-danger" data-action="staff-delete" data-id="${account.id}">Usuń konto</button><button class="btn btn-blue" data-action="staff-save" data-id="${account.id}">Zapisz konto</button></div></div></details>`;}
  ...
  for(const [id,context] of mapContexts){
    ...
    const drawZones=()=>{
      ...
      for(const zone of zones){
        ...
        shape?.bindTooltip(`<b>${esc(zone.name)}</b><br>${zone.type} · ${zone.team}<br>${zone.shape==='POLYGON'?`${zone.points?.length||0} punktów`:`${zone.radius} m`}`,{sticky:true});
...
async function switchAdminSession(gameId){
  try{const data=await api(`/api/state?gameId=${encodeURIComponent(gameId)}`);backend.gameId=gameId;assignSnapshot(data);sessionStorage.setItem('fm-admin-game-id',gameId);ui.boundaryDraft=null;ui.zoneDrafts=null;ui.zoneDraftGameId=null;ui.selectedMode=state.game.mode;ui.staffAccountsLoaded=false;await loadStaffAccounts();connectRealtime(backend.token,gameId);render();}catch(error){toast('Nie przełączono sesji',error.message,'critical');}
}
...
document.addEventListener('visibilitychange',()=>{if(!ui.fieldMode)return;if(document.visibilityState==='visible'){requestFieldWakeLock();if(enabled('reconnectRecovery')){manageGps(currentPlayer());if(backend.socket&&!backend.connected)backend.socket.connect();}}else showFieldNotification('Fieldmaster działa w tle','Nie zamykaj aplikacji. System może ograniczyć GPS po zablokowaniu ekranu.');});
...
async function saveGameplaySettings(e){e.preventDefault();const mode=$('input[name="game-mode"]:checked')?.value||state.game.mode,modeRules=Object.fromEntries($$('[data-mode-rule]',e.currentTarget).map(input=>[input.dataset.modeRule,input.type==='checkbox'?input.checked:Number(input.value)]));const payload={mode,modeSettings:{hitsToRespawn:Number($('#hits-to-respawn').value),respawnSeconds:Number($('#respawn-seconds').value),respawnZoneRequired:$('#respawn-zone-required').checked,lives:Number($('#mode-lives').value),scoreLimit:Number($('#score-limit').value),roundMinutes:Number($('#round-minutes').value),fovRange:Number($('#fov-range').value),fovAngle:Number($('#fov-angle').value),modeRules}};try{state.game=normalizeGame(await api(`/api/games/${activeGameId()}/settings`,{method:'PATCH',body:JSON.stringify(payload)}));ui.selectedMode=state.game.mode;toast('Tryb zapisany',`${GAME_MODES[state.game.mode]?.name||state.game.mode} jest teraz aktywny.`);render();}catch(error){toast('Nie zapisano trybu',error.message,'critical');}}
...
function newZoneDraft(){const controller=activeMaps.find(item=>item.context.options.zoneEdit),mapCenter=controller?.map.getCenter(),fallback=mapCenter?[mapCenter.lat,mapCenter.lng]:zoneCenterFromPoints(state.game.boundary);const zone={id:uid(),name:`Nowa strefa ${ensureZoneDrafts().length+1}`,type:'RESPAWN',team:'ALL',shape:'POLYGON',center:[...fallback],points:squarePointsAround(fallback),radius:100,color:'#3fa7ff'};ui.zoneDrafts.push(zone);ui.selectedZoneId=zone.id;render();}
function updateZoneDraftFromForm(){const zone=selectedZone();if(!zone)return;zone.name=$('#zone-name')?.value.trim()||zone.name;zone.type=$('#zone-type')?.value||zone.type;zone.team=$('#zone-team')?.value||zone.team;zone.radius=Math.max(10,Number($('#zone-radius')?.value||zone.radius||100));zone.color=$('#zone-color')?.value||zone.color;updateActiveMaps();}
...
function clearZonePoints(){const zone=selectedZone();if(!zone)return;zone.points=[];updateActiveMaps();const label=$('#zone-center-label');if(label)label.textContent='0 punktów wielokąta';}
function squareZoneDraft(){const zone=selectedZone();if(!zone)return;const controller=activeMaps.find(item=>item.context.options.zoneEdit),c=controller?.map.getCenter();if(c)zone.center=[c.lat,c.lng];zone.points=squarePointsAround(zone.center);updateActiveMaps();const label=$('#zone-center-label');if(label)label.textContent='4 punkty wielokąta';}
...
async function createStaffAccount(e){e.preventDefault();const permissions=$$('[data-new-permission]:checked',e.currentTarget).map(input=>input.dataset.newPermission);const payload={gameId:activeGameId(),username:$('#staff-username').value.trim(),callsign:$('#staff-callsign').value.trim(),title:$('#staff-title').value.trim(),password:$('#staff-password').value,team:$('#staff-team').value,permissions};try{const account=await api('/api/staff',{method:'POST',body:JSON.stringify(payload)});ui.staffAccounts=[...ui.staffAccounts.filter(item=>item.id!==account.id),account];ui.staffAccountsLoaded=true;state.staff=ui.staffAccounts;await loadStaffAccounts();toast('Konto utworzone',`${payload.callsign} może zalogować się kodem ${state.game.code}.`);render();}catch(error){toast('Nie utworzono konta',error.message,'critical');}}
async function saveStaffAccount(id){const account=ui.staffAccounts.find(item=>item.id===id);if(!account)return;const permissions=$$(`[data-staff-permission^="${id}:"]:checked`).map(input=>input.dataset.staffPermission.split(':')[1]);const password=$(`#staff-password-${id}`)?.value;const payload={callsign:$(`#staff-callsign-${id}`).value.trim(),title:$(`#staff-title-${id}`).value.trim(),team:$(`#staff-team-${id}`).value,active:Boolean($(`[data-staff-active="${id}"]`)?.checked),permissions};if(password)payload.password=password;try{const updated=await api(`/api/staff/${id}`,{method:'PATCH',body:JSON.stringify(payload)});ui.staffAccounts=ui.staffAccounts.map(item=>item.id===id?updated:item);state.staff=ui.staffAccounts;toast('Konto zapisane',updated.callsign);render();}catch(error){toast('Nie zapisano konta',error.message,'critical');}}
