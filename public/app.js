...
function stateUiSignature(){return JSON.stringify({game:[state.game.id,state.game.state,state.game.name,state.game.durationMinutes,state.game.sereSeconds,state.game.opforSeconds,state.game.startedAt,state.game.finishedAt,state.game.boundary],participants:state.participants.map(p=>[p.id,p.team,p.status,Boolean(p.outside),Boolean(p.activeSos),p.timerEnd,p.boundaryCount]),sos:state.sos.map(s=>[s.id,s.status]),eventCount:state.events.length});}
...
function destroyMaps(){while(activeMaps.length){try{activeMaps.pop().map.remove();}catch{}}mapContexts.clear();}
function render(){
...
function installButton(className='btn btn-sm btn-ghost'){return `<button class="${className}" data-action="install" ${ui.installed?'disabled':''}>${ui.installed?'✓ Zainstalowana':'↓ Zainstaluj aplikację'}</button>`;}
...
function renderAdmin(){
  ...
  $('#app').innerHTML=`<div class="shell"><header class="topbar">${brand()}<nav class="nav">${['dashboard:Dowodzenie','map:Mapa live','participants:Uczestnicy','events:Historia','report:Raport','settings:Ustawienia'].map(x=>{const[k,l]=x.split(':');return`<button class="${ui.adminTab===k?'active':''}" data-tab="${k}">${l}</button>`}).join('')}</nav><div class="top-actions">${connection()}${installButton()}<div class="avatar">GM</div></div></header>
...
function initMaps(){
  ...
  for(const [id,context] of mapContexts){
    ...
    const map=window.L.map(element,{zoomControl:true,attributionControl:true,preferCanvas:true,zoomAnimation:false,fadeAnimation:false,markerZoomAnimation:false}).setView(fallback,15);
    ...
    satellite.addTo(map);window.L.control.layers({'Satelita':satellite,'Mapa drogowa':streets},{},{position:'topright'}).addTo(map);
    ...
    const drawParticipants=()=>{
      ...
      for(const p of locatedNow){
        const tooltip=`<b>${esc(p.callsign)}</b><br>${p.team} · ${statusLabel(p.status)}<br>±${Math.round(p.accuracy||0)} m<br>${mgrs(p.lat,p.lon)}`;
    ...
    map.on('moveend zoomend',()=>drawUtmGrid(map,gridLayer));
    activeMaps.push({map,context,drawBoundary,drawParticipants});
...
function alarmSequence(){beep(920,.28,.22);setTimeout(()=>beep(680,.28,.24),320);setTimeout(()=>beep(1060,.5,.3),660);navigator.vibrate?.([300,120,300,120,700]);}
...
function onGpsPosition(original,pos){
  ...
  const outside=state.game.state==='ACTIVE'&&!pointInPolygon(me.lat,me.lon,state.game.boundary||polygon);setOutsideState(me,outside);
...
function onGpsError(me,error,highAccuracy){
  ...
  if(error.code!==1&&highAccuracy){gpsRestartTimer=setTimeout(()=>{gpsRestartTimer=null;startGpsWatch(currentPlayer()||me,false);},1200);toast('Przełączam tryb GPS','Dokładny GPS nie odpowiedział — próbuję trybu zgodnego.','warning');return;}
...
function startBoundaryAlarm(){if(boundaryAlarmTimer)return;boundaryAlarmSequence();boundaryAlarmTimer=setInterval(boundaryAlarmSequence,30000);}
function stopBoundaryAlarm(){if(boundaryAlarmTimer){clearInterval(boundaryAlarmTimer);boundaryAlarmTimer=null;}}
function boundaryAlarmSequence(){beep(760,.18,.15);setTimeout(()=>beep(540,.22,.18),230);navigator.vibrate?.([300,140,300]);}
...
async function initialize(){
  ...
  if(backend.available){
    if(ui.view==='admin'){
      ...
      if(token){try{assignSnapshot(await api('/api/state',{},token));backend.token=token;backend.gameId=state.game.id;connectRealtime(token,state.game.id);}catch{sessionStorage.removeItem('fm-admin');sessionStorage.removeItem('fm-admin-token');}}
