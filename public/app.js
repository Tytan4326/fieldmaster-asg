...
function renderAdmin(){
  ...
  const elapsed=state.game.startedAt?Date.now()-state.game.startedAt:0; const remaining=Math.max(0,state.game.durationMinutes*60000-elapsed);
...
function renderAdminLogin(){ $('#app').innerHTML=`<main class="join-shell"><section class="join-form-side">${brand()}<form class="join-form" id="admin-login"><div class="eyebrow">Bezpieczny dostęp</div><h1>Panel dowodzenia</h1><p class="lead">Zaloguj się jako organizator. W wersji demonstracyjnej użyj PIN-u 2468.</p><div class="field"><label class="label">KRYPTONIM</label><input class="input" value="GAME-MASTER" disabled></div><div class="field"><label class="label" for="admin-pin">PIN</label><input id="admin-pin" class="input" type="password" inputmode="numeric" maxlength="8" autocomplete="current-password" autofocus></div><button class="btn btn-primary" style="width:100%">Wejdź do centrum dowodzenia</button><p class="hint">W produkcji PIN demonstracyjny zastępuje hasło przechowywane jako hash, krótka sesja i opcjonalne MFA.</p></form></section><aside class="join-visual"><div class="visual-copy"><div class="visual-number">C2</div><h2>Pełna świadomość sytuacyjna.</h2><p>Jedno centrum dla lokalizacji, alarmów, statusów i decyzji organizatora.</p></div></aside></main>`; }
function gameActions(){ if(state.game.state==='LOBBY')return`<button class="btn btn-primary" data-game="start">${icons.play} Rozpocznij grę</button>`;if(state.game.state==='ACTIVE')return`<button class="btn" data-game="pause">${icons.pause} Wstrzymaj</button><button class="btn btn-danger" data-game="finish">${icons.stop} Zakończ</button>`;if(state.game.state==='PAUSED')return`<button class="btn btn-primary" data-game="resume">${icons.play} Wznów</button><button class="btn btn-danger" data-game="finish">${icons.stop} Zakończ</button>`;return`<button class="btn" data-action="reset-demo">↻ Nowa demonstracja</button>`; }
...
function handleAction(action,button){
  ...
  if(action==='reset-demo'){state=initialState();save();render();}
...
async function changeGame(action){
  const map={start:'ACTIVE',pause:'PAUSED',resume:'ACTIVE',finish:'FINISHED'};
  try{
    ...
    else{state.game.state=map[action];if(action==='start'){state.game.startedAt=Date.now();state.participants.forEach(p=>p.status='ACTIVE');}if(action==='finish'){state.game.finishedAt=Date.now();state.participants.forEach(p=>p.status='FINISHED');stopGps();}addEvent(`GAME_${map[action]}`,`GAME-MASTER: ${statusLabel(map[action]).toLowerCase()}`);}
    render();
