...
try {
  ...
  await evaluate(`(async()=>{const adminToken=sessionStorage.getItem('fm-admin-token'),gameId=document.querySelector('#admin-session')?.value,join=await fetch('/api/games/WILK24/join',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({callsign:'REPLAY-QA',team:'SERE',consent:true,consentVersion:'qa'})}),joined=await join.json();if(join.ok){await fetch('/api/games/'+gameId+'/start',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+adminToken},body:'{}'});for(const [i,point] of [[52.2304,21.0184],[52.2308,21.0190],[52.2312,21.0198]].entries())await fetch('/api/locations',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+joined.token},body:JSON.stringify({latitude:point[0],longitude:point[1],accuracy:4,heading:45,headingSource:'COMPASS',timestamp:new Date(Date.now()+i*6000).toISOString()})});}return join.ok})()`);await sleep(900);
