...
try {
  ...
  failures.push(...results.filter(item=>(item.playerMapLock&&(!item.playerMapLock.info||!item.playerMapLock.quickActions||!item.playerMapLock.quickInViewport))||(item.featureRecommendations&&(item.featureRecommendations.badges<5||!item.featureRecommendations.button))||(item.knowledge&&(item.knowledge.chapters<5||item.knowledge.nested<12))||(item.accountEditor&&(!item.accountEditor.revokeButton||!item.accountEditor.expiry||!item.accountEditor.customPreset||item.accountEditor.presetCards<5))).filter(item=>!failures.includes(item)));
