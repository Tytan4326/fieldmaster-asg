...
try {
  ...
  const playerMapLock=await evaluate(`(()=>{const first=document.querySelector('.quick-actions'),rect=first?.getBoundingClientRect();return{status:Boolean(document.querySelector('[data-player-tab="status"]')),info:Boolean(document.querySelector('[data-player-tab="info"]')),mapButton:Boolean(document.querySelector('[data-player-tab="map"]')),callsign:document.querySelector('.player-identity-card b')?.textContent,quickActions:Boolean(first),quickTop:rect?.top,quickInViewport:Boolean(rect&&rect.top<innerHeight)}})()`);await screenshot('player-map-locked-mobile');results.push({name:'player-map-locked-mobile',...(await inspect()),playerMapLock});
  await evaluate(`window.FieldmasterNative={getStatus:()=>JSON.stringify({native:true,version:'1.1.0',versionCode:2,fineLocation:true,coarseLocation:true,backgroundLocation:true,notifications:true,accessibility:true,batteryUnrestricted:true,fieldMode:true,session:true,serviceRunning:true,volumeUpEnabled:true,volumeUpAction:'TIMER',volumeDownEnabled:false,volumeDownAction:'HIT'}),configureSession:()=>{},setFieldMode:()=>{},configureHardwareButtons:()=>{},testHardwareButton:()=>{}};document.querySelector('[data-player-tab="info"]').click()`);await sleep(450);await evaluate(`document.querySelector('.remote-control-card').open=true;document.querySelector('.native-apk-card').open=true;document.querySelector('[data-action="remote-learn"][data-slot="A"]').click();window.dispatchEvent(new KeyboardEvent('keydown',{key:'b',code:'KeyB',keyCode:66,bubbles:true,cancelable:true}))`);await sleep(300);await screenshot('player-remote-settings-mobile');
  const remoteSettings=await evaluate(`(()=>({card:Boolean(document.querySelector('details.remote-control-card')),enabled:Boolean(document.querySelector('#remote-enabled')),mappings:document.querySelectorAll('[data-remote-action]').length,learn:document.querySelectorAll('[data-action="remote-learn"]').length,calibration:Boolean(document.querySelector('.compass-calibration-card')),stats:document.querySelectorAll('.calibration-stats>div').length,learned:JSON.parse(localStorage.getItem('fm-remote-settings')||'{}').keyA==='key:b',nativeCard:Boolean(document.querySelector('details.native-apk-card')),nativeToggles:document.querySelectorAll('[data-native-button-enabled]').length,nativeActions:document.querySelectorAll('[data-native-button-action]').length,upEnabled:document.querySelector('[data-native-button-enabled="up"]')?.checked,downEnabled:document.querySelector('[data-native-button-enabled="down"]')?.checked,currentUpdate:Boolean(document.querySelector('[data-action="native-update"]')),currentInstall:Boolean(document.querySelector('[data-action="install"]'))}))()`);results.push({name:'player-remote-settings-mobile',...(await inspect()),remoteSettings});
  await evaluate(`window.FieldmasterNative.getStatus=()=>JSON.stringify({native:true,version:'1.0.0',versionCode:1,volumeUpEnabled:true,volumeUpAction:'TIMER',volumeDownEnabled:false,volumeDownAction:'HIT'});window.dispatchEvent(new CustomEvent('fieldmaster:native-status'))`);await sleep(650);
  const nativeUpdateControl=await evaluate(`(()=>({update:Boolean(document.querySelector('[data-action="native-update"]')),install:Boolean(document.querySelector('[data-action="install"]')),label:document.querySelector('[data-action="native-update"]')?.textContent||''}))()`);results.push({name:'player-native-update-control',...(await inspect()),nativeUpdateControl});
  ...
  await evaluate(`document.querySelector('[data-settings-tab="features"]').click()`);await sleep(400);const featureRecommendations=await evaluate(`(()=>({badges:document.querySelectorAll('.recommended-badge').length,button:Boolean(document.querySelector('[data-action="features-recommended"]')),mode:document.querySelector('.feature-recommendation')?.parentElement?.querySelector('.panel-sub')?.textContent}))()`);await screenshot('admin-feature-recommendations-desktop');results.push({name:'admin-feature-recommendations-desktop',...(await inspect()),featureRecommendations});
  ...
  const knowledge=await evaluate(`(()=>({modes:document.querySelectorAll('.knowledge-list details').length,features:document.querySelectorAll('.feature-help article').length,zones:document.querySelectorAll('.zone-info-grid article').length,chapters:document.querySelectorAll('.knowledge-chapter').length,nested:document.querySelectorAll('.knowledge-chapter .chapter-body details').length}))()`);results.push({name:'admin-information-desktop',...(await inspect()),knowledge});
  ...
  await viewport(1440,1000);await navigate(`${base}/admin.html`);await sleep(1200);
  await evaluate(`document.querySelector('[data-tab="report"]').click()`);await sleep(1200);await evaluate(`document.querySelector('[data-action="archive-create"]')?.click()`);await sleep(2500);await screenshot('admin-archive-desktop');
  ...
  failures.push(...results.filter(item=>(item.playerMapLock&&(!item.playerMapLock.info||!item.playerMapLock.quickActions||!item.playerMapLock.quickInViewport))||(item.remoteSettings&&(!item.remoteSettings.card||!item.remoteSettings.enabled||item.remoteSettings.mappings!==2||item.remoteSettings.learn!==2||!item.remoteSettings.calibration||item.remoteSettings.stats!==4||!item.remoteSettings.learned||!item.remoteSettings.nativeCard||item.remoteSettings.nativeToggles!==2||item.remoteSettings.nativeActions!==2||item.remoteSettings.upEnabled!==true||item.remoteSettings.downEnabled!==false||item.remoteSettings.currentUpdate||item.remoteSettings.currentInstall))||(item.nativeUpdateControl&&(!item.nativeUpdateControl.update||item.nativeUpdateControl.install||!item.nativeUpdateControl.label.includes('1.1.0')))||(item.featureRecommendations&&(item.featureRecommendations.badges<5||!item.featureRecommendations.button))||(item.knowledge&&(item.knowledge.chapters<5||item.knowledge.nested<12))||(item.accountEditor&&(!item.accountEditor.revokeButton||!item.accountEditor.expiry||!item.accountEditor.customPreset||item.accountEditor.presetCards<5))).filter(item=>!failures.includes(item)));
