...
const inspect = () => evaluate(`(() => {
  const selectors='.topbar,.nav,.top-actions,.admin-main,.command-strip,.stats,.panel,.settings-shell,.settings-nav,.settings-category,.mode-grid,.zones-layout,.accounts-layout,.participants-console,.participant-filters,.time-settings-grid,.knowledge-base,.leaflet-map,.bottom-nav';
...
try {
  ...
  await viewport(1440,1000);await navigate(`${base}/admin.html`);await sleep(1200);
  await evaluate(`document.querySelector('[data-tab="report"]').click()`);await sleep(500);await evaluate(`document.querySelector('[data-action="replay-load"]')?.click()`);await sleep(1300);await screenshot('admin-replay-desktop');
