...
try {
  ...
  await evaluate(`document.querySelector('[data-player-tab="info"]').click()`);await sleep(350);await evaluate(`document.querySelector('[data-action="remote-learn"][data-slot="A"]').click();window.dispatchEvent(new KeyboardEvent('keydown',{key:'b',code:'KeyB',keyCode:66,bubbles:true,cancelable:true}))`);await sleep(300);await screenshot('player-remote-settings-mobile');
  const remoteSettings=await evaluate(`(()=>({card:Boolean(document.querySelector('.remote-control-card')),enabled:Boolean(document.querySelector('#remote-enabled')),mappings:document.querySelectorAll('[data-remote-action]').length,learn:document.querySelectorAll('[data-action="remote-learn"]').length,calibration:Boolean(document.querySelector('.compass-calibration-card')),stats:document.querySelectorAll('.calibration-stats>div').length,learned:JSON.parse(localStorage.getItem('fm-remote-settings')||'{}').keyA==='key:b'}))()`);results.push({name:'player-remote-settings-mobile',...(await inspect()),remoteSettings});
